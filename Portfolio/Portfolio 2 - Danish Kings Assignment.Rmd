...
author: "Sofie Teilmann-Jørgensen"
date: "7 march 2025"
...
kings1 <- read.csv("data/kongerække2.csv")
glimpse(kings1)
head(kings1)

kings2 <- read_csv("data/kongerække2.csv")
glimpse(kings2)
kings2

kings3 <- read.csv2("data/kongerække2.csv")
glimpse(kings3)

kings4 <- read_csv2( "data/kongerække2.csv")
glimpse(kings4)
```
Answer: 
...
duration <- kings$duration

duration_no_nas <- duration[!is.na(duration)]

mean(duration_no_nas)
```
...
kings_longer_than_average <- kings %>% 
  filter(duration_no_nas > 19.76364)
...
descending_duration <- kings %>%  
  arrange(desc(duration))

descending_duration %>% slice_head(n = 3)
